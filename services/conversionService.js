...
<DL><p>
    ...
    if (result.xbel.folder) {
      result.xbel.folder.forEach((folder) => {
        if (folder.$ && folder.title && folder.title[0]) {
          html += `        <DT><H3 ADD_DATE="0" LAST_MODIFIED="0">${escapeHtml(
            folder.title[0]
          )}</H3>\n`;
          html += `        <DL><p>\n`;
          ...
          if (folder.bookmark) {
            folder.bookmark.forEach((bookmark) => {
              if (bookmark.$ && bookmark.$.href) {
                const title =
                  bookmark.title && bookmark.title[0] ? bookmark.title[0] : "";
                html += `            <DT><A HREF="${
                  bookmark.$.href
                }" ADD_DATE="0">${escapeHtml(title)}</A>\n`;
              }
  ...
  static async processConversion(fileBuffer, originalFilename) {
    try {
      ...
      const savedFilename = FileModel.saveUploadedFile(
        fileBuffer,
        originalFilename
      );
      const convertedFilename = FileModel.saveConvertedFile(
        htmlContent,
        originalFilename
      );
      ...
      return {
        ...
        originalName: path.parse(originalFilename).name,
      };
    } catch (error) {
      // Ensure we clean up any locked files if conversion fails
      cacheService.removeFromCache(originalFilename);
      throw error;
    }
  }
}
